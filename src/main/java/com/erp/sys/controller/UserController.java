...
import com.erp.sys.common.PinyinUtils;
...
//        queryWrapper.eq(userVo.getDeptid()!=null,"deptid",userVo.getDeptid());
        if(userVo.getDeptid()!=null){
            ...
            deptQueryWrapper.eq("id",userVo.getDeptid()).or().in("pid",userVo.getDeptid());
            List<Dept> list = this.deptService.list(deptQueryWrapper);
            Collection<Serializable> idList = new ArrayList<>();
            for (Dept dept: list) {
                idList.add(dept.getId());
            }
            queryWrapper.in("deptid",idList);
        }
        ...
        for (User user: records) {
            QueryWrapper<Dept> deptQueryWrapper = new QueryWrapper<>();
            deptQueryWrapper.eq(user.getDeptid()!=null,"id",user.getDeptid());
            user.setDeptname(deptService.getOne(deptQueryWrapper).getTitle());

            if(user.getMgr()!=null){
                QueryWrapper<User> userQueryWrapper = new QueryWrapper<>();
                userQueryWrapper.eq("id",user.getMgr());
                user.setLeadername(userService.getOne(userQueryWrapper).getName());
            }
        }
